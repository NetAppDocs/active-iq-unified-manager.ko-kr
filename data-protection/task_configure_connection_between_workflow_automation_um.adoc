...
.시작하기 전에
* Workflow Automation의 설치 버전은 5.1.1P6 이상이어야 합니다.
+
[NOTE]
====
WFA 5.1.1P6에 "'Clustered Data ONTAP 관리를 위한 WFA 팩'이 포함되어 있으므로 NetAppStorage 자동화 스토어에서 이 팩을 다운로드하여 WFA 서버에 별도로 설치할 필요가 없습니다. https://automationstore.netapp.com/pack-list.shtml["ONTAP 관리를 위한 WFA 팩"]
