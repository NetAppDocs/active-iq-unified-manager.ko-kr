...
---
= Red Hat Enterprise Linux에 Active IQ Unified Manager 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
|===
. 설치된 MySQL 버전이 MySQL 8.4.10 Community Edition 이전 버전인 경우, 다음 명령을 입력하여 제거하십시오.
+
...
+
Red Hat Enterprise Linux 8 시스템에서 JDK 17 - OpenJDK, 7-zip 및 AppStream 저장소에서 제공하는 기타 소프트웨어 패키지가 포함된 내부 저장소가 있는 경우 AppStream 저장소를 비활성화하고 MySQL Community Server를 설치해야 합니다. 다음 명령어를 실행하세요:
