...
+
.. 다음 명령을 실행하여 컨텍스트를 mysqld_db_t로 설정합니다. semanage fcontext-a-t mysqld_db_t"/opt/netapp/data"""restorecon-R-v/opt/netapp/data"
...
. 다음 단계를 수행하여 '/opt/NetApp'을(를) 마운트한 방법에 따라 컨텍스트를 설정합니다.
+
.. 다음 명령을 실행하여 컨텍스트를 mysqld_db_t로 설정합니다. semanage fcontext-a-t mysqld_db_t"/opt/netapp""restorecon-R-v/opt/netapp"
