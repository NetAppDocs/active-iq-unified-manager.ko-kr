...
---
= Active IQ Unified Manager가 VMware 가상 인프라를 모니터링하는 방법 이해
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image::../media/vm_deployment.gif[VMware 호스트, VM, 데이터스토어, VMDK, IP 네트워크 및 스토리지 연결을 보여주는 다이어그램]

* 사용자 워크플로 *

다음 다이어그램에서는 VM 토폴로지 뷰를 사용하는 일반적인 사용 사례를 보여 줍니다.

image::../media/vm_workflow.gif[컴퓨팅, 네트워크 및 스토리지 계층 전반에 걸쳐 VM 애플리케이션 지연 시간을 찾는 워크플로를 보여주는 다이어그램]
