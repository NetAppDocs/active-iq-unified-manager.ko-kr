...
---
= Active IQ Unified Manager에서 MySQL 데이터베이스 덤프를 사용하여 백업 및 복원
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image::../media/database_backup.gif[데이터베이스 저장소의 전체 백업 파일과 증분 백업 파일을 가리키는 세 개의 백업 디렉터리 파일을 보여주는 다이어그램]
